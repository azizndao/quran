...
import org.alquran.audio.PlaybackService
...
class PlaybackConnection(
    ...
    fun getTimingFlow(delay: Long = 200) = _currentMediaItem.flatMapLatest { mediaItem ->
        flow {
            while (true) {
    ...
    val currentAyah = getTimingFlow().distinctUntilChangedBy { it?.ayah }
        .stateIn(coroutineScope, SharingStarted.WhileSubscribed(), null)

    val nowPlaying = getTimingFlow(100).combine(_playingState) { timing, state ->
        ...
        NowPlaying(
            title = mediaMetadata.title.toString() + " (${timing.sura}:${timing.ayah})",
    ...
    suspend fun onPlaySurah(
        ...
        val mediaItems = recitationRepository.getRecitations(reciter.id)
        val position = timingRepository.getPosition(reciter.id, surah, startAyah)
