...
import org.quram.common.utils.QuranConstants.LAST_SURA
import org.quram.common.utils.QuranConstants.MAX_AYAH
import org.quram.common.utils.QuranConstants.MIN_AYAH
import org.quram.common.utils.QuranConstants.NUMBER_OF_SURAS
import kotlin.math.abs

class QuranInfo internal constructor(quranDataSource: QuranDataSource) {

  companion object {
    fun getInstance(version: Int): QuranInfo = QuranInfo(QuranDataSource.getInstance(version))
  }
  ...
  val quarters: Array<VerseKey> = quranDataSource.quartersArray
  val juzsArray = quranDataSource.juzsArray
  ...
  fun diff(start: VerseKey, end: VerseKey): Int {
    return getAyahId(end.sura, end.aya) - getAyahId(start.sura, start.aya)
  }
  ...
  fun getJuzFromSurahAyah(sura: Int, ayah: Int, juz: Int): Int {
    ...
    return if (sura > lastQuarter.sura || lastQuarter.sura == sura && ayah >= lastQuarter.aya) {
      juz + 1
    } else {
