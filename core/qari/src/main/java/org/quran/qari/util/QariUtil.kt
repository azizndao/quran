...
import org.quran.qari.R
import org.quran.qari.model.QariItem

class QariUtil internal constructor(private val context: Context) {
   ...
   */
  fun getQariList(): List<QariItem> {
    return getQariItemList().map { item ->
      QariItem(
        id = item.id,
